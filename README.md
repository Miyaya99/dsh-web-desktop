...
> **重复点击会问你要干什么**：服务没在跑时，点图标就是启动；服务**已经在跑**时，会弹一个三按钮对话框问你 —— **Yes** 重启服务（先停再起，改了插件用这个）、**No** 保留当前服务另开一个窗口、**Cancel** 什么都不做（25 秒不选也按"不动"处理）。
>
> 这么做是因为图标没有参数，而第二次点击只可能有两种意思。对话框文案是英文的：启动器由 `powershell.exe`（Windows PowerShell 5.1）执行，它把无 BOM 的 `.ps1` 当 ANSI 读，中文会乱码，安装脚本写回文件时也是无 BOM 的 UTF-8 —— 所以启动器里所有字符串都必须是 ASCII。命令行想跳过弹窗可以用 `-Restart` / `-New` / `-NoPrompt`。
...
2. 端口上已经有 dsh 在跑 → **弹窗问你**：重启服务，还是保留它另开一个窗口；
3. 端口被**别的程序**占用 → 弹窗提示，不会瞎启动；
4. 选了"重启" → 先停掉端口上的 dsh（确认是 node 才动手），**等端口真正释放**再重新启动，最后开界面。

**停止**：点 **DSH Web (Stop)**（或命令行 `-Stop`）。
**重启**：再点一次 **DSH Web**，在弹窗里选 **Yes**（或直接命令行 `-Restart`）—— 改了插件、想重新加载时用这个。
